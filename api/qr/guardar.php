...
// Autorización similar a qr/generar.php
...
$url = isset($body['url']) && is_string($body['url']) ? trim($body['url']) : null;

if (!in_array($area, ['interior','exterior'], true) || $mesaId === '' || $path === '') {
...
try {
  ...
  // Upsert
  $st = db()->prepare('INSERT INTO qr_mesas (area, mesa_id, url, path) VALUES (:area, :mesa_id, :url, :path)
    ON DUPLICATE KEY UPDATE url = VALUES(url), path = VALUES(path), updated_at = CURRENT_TIMESTAMP');
  $st->execute([
    ':area' => $area,
    ':mesa_id' => $mesaId,
    ':url' => $url,
