...
export class MapaSalonApiService {
  ...
  getLayout(area: 'interior' | 'exterior'): Observable<any[]> {
    ...
    return this.http
      .get<{
        ...
        data: any[];
        updated_at?: string;
      }>(`${BASE_URL}/layouts.php`, { params })
      .pipe(map((res) => res?.data ?? []));
  }

  saveLayout(area: 'interior' | 'exterior', mesas: any[]): Observable<boolean> {
    const params = new HttpParams().set('area', area);
    return this.http.put(`${BASE_URL}/layouts.php`, mesas, { params }).pipe(map(() => true));
  }

  // ===== Mesas / Pedidos =====
  getEstadoMesas(
    area: 'interior' | 'exterior',
  ): Observable<{ mesaId: string; estado: 'libre' | 'ocupada'; pedidoId?: number | null }[]> {
    const params = new HttpParams().set('area', area);
    return this.http
  ...
  abrirMesa(
    area: 'interior' | 'exterior',
    mesaId: string,
  ): Observable<{ ok: boolean; pedidoId: number }> {
    const token = typeof window !== 'undefined' ? localStorage.getItem('auth_token') || '' : '';
    return this.http.post<{ ok: boolean; pedidoId: number }>(`${BASE_URL}/mesas/abrir.php`, {
      area,
      mesaId,
      token,
    });
  ...
  agregarItem(pedidoId: number, productoId: number, cantidad = 1): Observable<{ ok: boolean }> {
    const token = typeof window !== 'undefined' ? localStorage.getItem('auth_token') || '' : '';
    return this.http.post<{ ok: boolean }>(`${BASE_URL}/mesas/agregar-item.php`, {
      pedidoId,
      productoId,
      cantidad,
      token,
  ...
  pagarPedido(
    pedidoId: number,
    metodo: 'efectivo' | 'tarjeta' | 'qr' | 'mixto' = 'efectivo',
    monto?: number,
  ): Observable<{ ok: boolean }> {
    const token = typeof window !== 'undefined' ? localStorage.getItem('auth_token') || '' : '';
    return this.http.post<{ ok: boolean }>(`${BASE_URL}/mesas/pagar.php`, {
      pedidoId,
      metodo,
      monto,
      token,
  ...
  liberarMesa(area: 'interior' | 'exterior', mesaId: string): Observable<{ ok: boolean }> {
    const token = typeof window !== 'undefined' ? localStorage.getItem('auth_token') || '' : '';
    return this.http.post<{ ok: boolean }>(`${BASE_URL}/mesas/liberar.php`, {
      area,
      mesaId,
      token,
    });
  ...
  listarProductos(): Observable<{ categorias: any[]; productos: any[] }> {
    return this.http.get<{ categorias: any[]; productos: any[] }>(
      `${BASE_URL}/productos/listar.php`,
    );
  }
}
