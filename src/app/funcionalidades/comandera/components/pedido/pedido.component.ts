  ...
  <div class="min-h-[100svh] bg-gradient-to-br from-orange-50 via-white to-orange-100 flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto px-3 py-3 md:px-6 md:py-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2">
          <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-md ring-1 ring-orange-100/60 overflow-hidden">
            <div class="px-4 py-3 border-b text-gray-800 font-semibold flex items-center gap-2 bg-gradient-to-r from-white to-orange-50/50">
              <mat-icon class="text-orange-600">list_alt</mat-icon>
              Ítems
            </div>
            <div class="p-2 divide-y divide-gray-100">
              <div *ngFor="let it of (pedido()?.items || [])" class="flex items-center justify-between gap-3 py-3 px-2 hover:bg-orange-50/60 transition-colors">
                <div class="min-w-0 flex items-start gap-3">
                  <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-100 to-orange-200 text-orange-700 flex items-center justify-center ring-1 ring-orange-200">
                ...
                <div class="flex items-center gap-3">
                  <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-orange-100 text-orange-700 text-sm font-bold ring-1 ring-orange-200 shadow">{{ it.cantidad }}</span>
        ...
        <div>
          ...
          <!-- Resumen -->
          <div class="mt-4 bg-white/80 backdrop-blur-xl rounded-2xl shadow-md ring-1 ring-orange-100/60 overflow-hidden">
            <div class="px-4 py-3 border-b text-gray-800 font-semibold flex items-center gap-2 bg-gradient-to-r from-white to-orange-50/50">
    ...
    <div class="sticky bottom-0 z-40 bg-white/90 backdrop-blur-xl border-t shadow-inner" *ngIf="!isPagado()">
      <div class="px-3 py-3 grid grid-cols-3 gap-2 md:max-w-3xl md:mx-auto">
        ...
        <button mat-stroked-button color="accent" class="!h-12 !font-semibold" (click)="confirmarPago('qr')"><mat-icon>qr_code</mat-icon>&nbsp;QR</button>
      </div>
    </div>
    ...
    <ng-template #dlgConfirmCobro>
      <div class="p-5 pt-4 max-w-[420px]">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button mat-raised-button color="primary" (click)="dialogRef?.close(true)" class="!h-11 !rounded-xl">
            <mat-icon>done</mat-icon>&nbsp;Sí, cobrado
          </button>
          <button mat-stroked-button color="warn" (click)="dialogRef?.close(false)" class="!h-11 !rounded-xl">
    ...
    <ng-template #dlgPostPago>
      <div class="p-5 pt-4 max-w-[420px]">
        ...
        <p class="text-sm text-gray-600 mb-4">¿Cómo querés continuar con la mesa {{ pedido()?.pedido?.mesa_id }}?</p>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button mat-raised-button color="primary" (click)="dialogRef?.close('liberar')" class="!h-11 !rounded-xl">
            <mat-icon>check_circle</mat-icon>&nbsp;Terminar y liberar
          </button>
          <button mat-stroked-button color="primary" (click)="dialogRef?.close('seguir')" class="!h-11 !rounded-xl">
      ...
      <div class="p-5 pt-4 max-w-[520px]">
        ...
        <div class="text-sm text-gray-700 space-y-2">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="font-medium">{{ pedido()?.pedido?.mesa_id }} ({{ pedido()?.pedido?.area }})</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-600">Ítems</span>
          ...
          <div class="mt-3 p-3 rounded-xl bg-orange-50 ring-1 ring-orange-200">
            ...
            <div class="flex items-center justify-between text-sm">
              ...
              <span class="text-[11px] px-2 py-0.5 rounded-full bg-orange-200/60 text-orange-900 uppercase tracking-wide">{{ mozoRol }}</span>
            </div>
          </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-2 mt-4">
          <button mat-stroked-button color="primary" class="!h-11 !rounded-xl sm:col-span-1" (click)="dialogRef?.close()">
            <mat-icon>close</mat-icon>&nbsp;Cerrar
          </button>
          <button mat-raised-button color="accent" class="!h-11 !rounded-xl sm:col-span-1" (click)="confirmarPagoDesdeFactura('efectivo')">
            <mat-icon>attach_money</mat-icon>&nbsp;Efectivo
          </button>
          <button mat-raised-button color="accent" class="!h-11 !rounded-xl sm:col-span-1" (click)="confirmarPagoDesdeFactura('tarjeta')">
            <mat-icon>credit_card</mat-icon>&nbsp;Tarjeta
          </button>
          <button mat-raised-button color="accent" class="!h-11 !rounded-xl sm:col-span-1" (click)="confirmarPagoDesdeFactura('qr')">
